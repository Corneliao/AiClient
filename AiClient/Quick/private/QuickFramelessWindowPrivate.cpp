...
#include "QuickFramelessWindowPrivate.h"
...
int QuickFramelessWindowPrivate::adjustResizeWindow(HWND const &hwnd, const int &x, const int &y) {
  ...
  if (resize_width) {
    if (x >= rect.left && x < rect.left + RESIZEWINDOWBORDERWIDTH) result = HTLEFT;
    if (x <= rect.right && x > rect.right - RESIZEWINDOWBORDERWIDTH) result = HTRIGHT;
  }
  if (resize_height) {
    if (y >= rect.top && y < rect.top + RESIZEWINDOWBORDERWIDTH) result = HTTOP;
    if (y <= rect.bottom && y > rect.bottom - RESIZEWINDOWBORDERWIDTH) result = HTBOTTOM;
  }
...
int QuickFramelessWindowPrivate::TackingWindowTitleBar(const QPoint &point) {
  int result = 0;
  if (this->titleBar == Q_NULLPTR) return result;
  QPointF pointF = this->titleBar->mapFromGlobal(QPointF(point.x() / this->dpr, point.y() / dpr));
  if (!this->titleBar->contains(pointF)) return result;
  QQuickItem *child = this->titleBar->childAt(pointF.x(), pointF.y());
  if (!child)
    result = HTCAPTION;
  else {
    if (this->titleWhiteListItem.contains(child)) result = HTCAPTION;
  }
 ...
 * @param bottom_margin
 */
void QuickFramelessWindowPrivate::adjustWindowMargin(const int &left_margin, const int &top_margin, const int &right_margin,
                                                     const int &bottom_margin) {
  Q_Q(QuickFramelessWindow);
  QQuickItem *layout_item = q->contentItem()->childItems().at(0);
  if (!layout_item) return;
  QMetaObject::invokeMethod(layout_item, "adjustWindowMargins", Qt::DirectConnection, Q_ARG(QVariant, left_margin),
                            Q_ARG(QVariant, top_margin), Q_ARG(QVariant, right_margin), Q_ARG(QVariant, bottom_margin));
}
...
void QuickFramelessWindowPrivate::ignoreTitleItem(QQuickItem *item) {
  if (!item) return;
  if (this->titleWhiteListItem.contains(item)) return;
